...
CREATE OR REPLACE FUNCTION auto_link_users_to_companies()
RETURNS jsonb AS $$
DECLARE
...
BEGIN
  ...
  FOR user_record IN (
    ...
    IF company_id IS NOT NULL THEN
      ...
      SET 
        ...
        company_role = CASE 
          WHEN user_record.company LIKE '%Admin%' OR user_record.company LIKE '%(Admin)' THEN 'admin'
          ELSE 'member'
        END
  ...
  RETURN jsonb_build_object(
    ...
    'message', format('Successfully linked %s users to companies', linked_count)
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  -- Get all companies with member counts from both foreign key AND text field matching
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', c.id,
      'name', c.name,
  ...
  LEFT JOIN (
    -- Count members linked by text field (case-insensitive)
    SELECT 
      find_company_by_name(company) as company_id,
      COUNT(*) as count
    FROM profiles
    WHERE company IS NOT NULL 
    AND company != ''
    AND company_id IS NULL  -- Only count those not already linked by FK
    GROUP BY find_company_by_name(company)
...
-- Function to automatically link existing users to companies
...
GRANT EXECUTE ON FUNCTION public.auto_link_users_to_companies() TO public;
