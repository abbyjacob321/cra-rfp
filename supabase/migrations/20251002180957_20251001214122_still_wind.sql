/*
  # Fix User-Company Assignment Functions and Member Counting
  
  1. Issues Fixed
    - Create missing admin_assign_user_to_company function
    - Fix parameter order and function signatures
    - Add proper foreign key relationship creation
    - Fix member count queries to handle both text and FK relationships
...
BEGIN
  ...
  FOR user_record IN (
    ...
    IF company_id IS NOT NULL THEN
      ...
      -- Log the linking (only if analytics_events table exists)
      ...
      END IF;
    ELSE
      -- Log companies that couldn't be matched (only if analytics_events table exists)
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'analytics_events') THEN
        INSERT INTO analytics_events (
          event_type,
          user_id,
...
BEGIN
  ...
  -- Notify user (only if notifications table exists)
